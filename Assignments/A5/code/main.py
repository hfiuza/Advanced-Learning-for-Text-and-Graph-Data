...
def bidir_gru(my_seq, n_units):
    ...
    '''
    return Bidirectional(GRU(units=n_units, return_sequences=True)### fill the gap ### # add a default GRU layer (https://keras.io/layers/recurrent/). You need to specify only the 'units' and 'return_sequences' arguments
...
bigru = bidir_gru(sent_wv_dr, n_units)
attention, attention_coeffs = AttentionWithContext(return_coefficients=True)(bigru)
sent_att_vec_dr = Dropout(drop_rate)(attention)
...
time_distributed = TimeDistributed(Dense(8), input_shape=(my_docs_array_train.shape[1], my_docs_array_train.shape[2]))(doc_ints)
doc_bigru = bidir_gru(time_distributed, n_units)
doc_attention, doc_attention_coeffs = AttentionWithContext(return_coefficients=True)(doc_bigru)
doc_att_vec_dr = Dropout(drop_rate)(doc_attention)
...
# apply the 'get_word_att_coeffs' model over all the sentences in 'my_review', and store the results as 'word_coeffs'
word_coeffs = np.array([get_word_att_coeffs.predict((my_review[:,idx,:])) for idx in range(int(my_review.shape[1]))])

...
path_to_data = '../data/'  # fill me!
path_to_word2vec = '../../A3/'  # fill me!
...
x_train = [[rev[idx] if idx < len(rev) else padding_idx for idx in range(max_size)] for rev in x_train]
x_test = [[rev[idx] if idx < len(rev) else padding_idx for idx in range(max_size)] for rev in x_test]
...
model.fit(np.array(x_train), np.array(np_utils.to_categorical(y_train, num_classes=2)), validation_split=0.1, epochs=nb_epochs, batch_size=batch_size, callbacks=[early_stopping, checkpointer])

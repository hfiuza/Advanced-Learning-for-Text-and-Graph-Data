...
all_terms = [term for terms in terms_by_doc for term in terms]  ### fill the gap ### hint: flatten 'terms_by_doc' (you may use a list comprehension)
...
for counter,unique_term in enumerate(list(idf.keys())):
    ...
    df = sum([int(unique_term in terms_set) for terms_set in terms_by_doc_sets]) ### fill the gap ###
    ...
    # idf
    idf[unique_term] = math.log10((len(terms_by_doc) + 1) / df) ### fill the gap ### hint: use math.log10 and refer to the beginning of Section 2 in the handout
    if counter % 1e3 == 0:
...
c_g = terms_to_graph(terms_by_doc, w, overspanning=True)### fill the gap ### hint: use the terms_to_graph function with the proper arguments
...
collection_degrees = {node['name']: node.degree() for node in c_g.vs}  ### fill the gap ### hint: build a dict where the keys are the names of the nodes in the collection graph and the values are their unweighted degrees
...
print('10 most important words for each class:')
print_top10(all_unique_terms, classifier_tfidf, unique_labels)
print('10 least important words for each class:')

...
for counter, abstract in enumerate(abstracts_cleaned):
    ...
    # create graph-of-words
    g = terms_to_graph(abstract, w=4)
    # compute core numbers with page rank
    pageranks = dict(zip(g.vs['name'], g.pagerank()))
    # keep only non zero values (the words in the document)
    nonzero = list(filter(lambda pr: pr[1] != 0, pageranks.items()))
    # rank by increasing pageranks

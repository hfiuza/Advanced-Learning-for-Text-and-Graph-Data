...
def to_parallelize(doc,collection,w2v):
    ...
    for doc_train in collection:
        to_return.append(w2v.wv.wmdistance(doc, doc_train)) ### fill gap ### append the Word Mover's Distance between doc and doc_train
    return to_return
...
word_and_norms = [(word, np.linalg.norm(w2v[word])) for word in vocab]
idxs_zero_norms = [idx for idx, word_and_norm in enumerate(word_and_norms) if word_and_norm [1] <= 0.05]
## fill the gap ## get the words with close to zero norms
no_entry_words =[word_and_norms[idx][0] for idx in idxs_zero_norms]
...
for idx,doc in enumerate(cleaned_docs):
    ...
    centroid = w2v.wv[doc].mean(axis=0)
